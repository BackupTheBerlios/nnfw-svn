...
public:
     ...
     *  the whole vec.
     */
    MatrixData( Vec& src, unsigned int rstart, unsigned int rend, unsigned int rows, unsigned int cols )
    ...
    public:
        /*! create the iterator */
        matrixdataIterator( MatrixData& data, unsigned int startId = 0 ) : vecdata(data.data), id(startId), tsize(data.tsize) { /*nothing to do*/ };
        /*! Copy-Constructor */
